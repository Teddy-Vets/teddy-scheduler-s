...
export default function TechHoursCard({ clinics, selectedClinicId, periodMode = "month", periodOffset = 0, shifts = [], staff = [] }) {
  const today = new Date();
  const staffMap = useMemo(() => Object.fromEntries((staff || []).map((s) => [s.id, s])), [staff]);

  const { totalClinicHours, totalTechHoursPlanned, label } = useMemo(() => {
    ...
    if (periodMode === "week") {
      ...
      for (const clinic of relevantClinics) {
        ...
        totalTechHoursPlanned += hours.tech;
      }
    } else {
    ...
    return { totalClinicHours: Math.round(totalClinicHours), totalTechHoursPlanned: Math.round(totalTechHoursPlanned), label };
  }, [clinics, selectedClinicId, periodMode, periodOffset]);

  const actualTechHours = useMemo(() => {
    const techShifts = (shifts || []).filter((s) => {
      ...
      return shiftRole === "tech" || shiftRole === "technician" || shiftRole === "surgery_tech";
    });
    return Math.round(techShifts.reduce((acc, s) => acc + shiftHours(s.start_time, s.end_time), 0));
  }, [shifts, staffMap]);

  const plannedRatio = totalClinicHours > 0 ? Math.round((totalTechHoursPlanned / totalClinicHours) * 100) : 0;
  const actualRatio = totalClinicHours > 0 ? Math.round((actualTechHours / totalClinicHours) * 100) : 0;

  return (
    <div className="bg-card border rounded-xl p-5 shadow-sm">
      <div className="flex items-center gap-2 mb-4">
        <HeartPulse className="w-5 h-5 text-primary" />
        <h3 className="font-semibold">שעות טכנאים לעומת שעות פעילות ({label})</h3>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        ...
        <div className="bg-primary/5 rounded-lg p-3 text-center border border-primary/20">
          <p className="text-xs text-muted-foreground mb-1">מתוכנן (טכנאים)</p>
          <p className="text-2xl font-bold text-primary">{totalTechHoursPlanned}</p>
          <p className="text-[10px] text-muted-foreground">שעות</p>
        </div>
        <div className="bg-green-50 rounded-lg p-3 text-center border border-green-200">
          <p className="text-xs text-muted-foreground mb-1">בפועל (טכנאים)</p>
          <p className="text-2xl font-bold text-green-700">{actualTechHours}</p>
          <p className="text-[10px] text-muted-foreground">שעות</p>
        </div>
      </div>
